...
const PLANS = {
  ...
  invest: [
    { id: "invest-small", unlockDay: 1, cost: 2, successRate: 0.6, successReturn: 4, failReturn: 0, shortLabel: "小さな話", detailLabel: "-2 / 60%で +4" },
    { id: "invest-mid", unlockDay: 5, cost: 4, successRate: 0.5, successReturn: 8, failReturn: 1, shortLabel: "うまそうな話", detailLabel: "-4 / 50%で +8" },
    { id: "invest-large", unlockDay: 10, cost: 6, successRate: 0.4, successReturn: 14, failReturn: 2, shortLabel: "大きな話", detailLabel: "-6 / 40%で +14" }
  ]
};
...
const newState=()=>({version:2,status:"idle",day:0,broughtIn:0,hand:0,totalSnackPaid:0,actionsLeft:ACTIONS_PER_DAY,animals:1,lends:[],borrows:[],investReturns:[],investResults:[],actionLogs:[],tired:false,lastDayIncoming:0,lastDayRepay:0});
...
function beginDay(s){if(s.status!=="playing")return; s.day+=1;s.actionsLeft=ACTIONS_PER_DAY;s.tired=false;const beforeAnimals=s.animals;s.animals=getAnimalsForDay(s.day);s.lastDayIncoming=getTodayIncomingAmount(s);s.lastDayRepay=getTodayRepaymentAmount(s);if(beforeAnimals>0&&s.animals>beforeAnimals)appendLog(s,`おやつを待つ動物が増えました。今日から${s.animals}匹です。`); s.lends=s.lends.filter(i=>i.day!==s.day||(s.hand+=i.amount,appendLog(s,`今日受け取った: +${i.amount}`),false)); s.investReturns=s.investReturns.filter(i=>i.day!==s.day||(s.hand+=i.amount,appendLog(s,`今日受け取った: +${i.amount}`),false)); let repay=0;s.borrows=s.borrows.filter(i=>i.day!==s.day||(repay+=i.amount,false)); if(repay){s.hand-=repay;appendLog(s,`今日返した: -${repay}`);} const snack=getDailySnackCost(s); s.hand-=snack;s.totalSnackPaid+=snack;appendLog(s,`今日のおやつ代として${snack}枚使いました。`); if(s.hand<=0){s.status="ended";s.tired=true;appendLog(s,"クッキーが尽きた。静かな夜が来る。");return;} appendLog(s,"今日もなんとか朝を迎えた。"); }
