...
const WORK_PLANS = [{ id: "gather_leaves", unlockDay: 1, label: "森で働く（労働）", rewardAmount: 2, display: "+2 / すぐ受け取る" }];

const PLANS = {
  lend: [
    { id: "lend-small", unlockDay: 1, cost: 4, dueDays: 2, returnAmount: 7, shortLabel: "短くあずける", detailLabel: "-4 / 2日後 +7" },
    { id: "lend-mid", unlockDay: 5, cost: 7, dueDays: 5, returnAmount: 13, shortLabel: "しばらくあずける", detailLabel: "-7 / 5日後 +13" },
    { id: "lend-large", unlockDay: 10, cost: 10, dueDays: 10, returnAmount: 20, shortLabel: "長くあずける", detailLabel: "-10 / 10日後 +20" }
  ],
  borrow: [
    { id: "borrow-small", unlockDay: 1, gain: 5, dueDays: 2, repayAmount: 7, shortLabel: "少し前借り", detailLabel: "+5 / 2日後 -7" },
    { id: "borrow-mid", unlockDay: 5, gain: 9, dueDays: 5, repayAmount: 14, shortLabel: "しばらく前借り", detailLabel: "+9 / 5日後 -14" },
    { id: "borrow-large", unlockDay: 10, gain: 15, dueDays: 10, repayAmount: 25, shortLabel: "長く前借り", detailLabel: "+15 / 10日後 -25" }
  ],
  invest: [
    { id: "invest-small", type: "instant_risk", unlockDay: 1, cost: 3, successRate: 0.65, successReturn: 7, failReturn: 0, shortLabel: "小さな話", detailLabel: "-3 / 65%で +7" },
    { id: "invest-mid", type: "instant_risk", unlockDay: 5, cost: 6, successRate: 0.5, successReturn: 14, failReturn: 0, shortLabel: "うまそうな話", detailLabel: "-6 / 50%で +14" },
    { id: "invest-large", type: "instant_risk", unlockDay: 10, cost: 10, successRate: 0.35, successReturn: 28, failReturn: 0, shortLabel: "大きな話", detailLabel: "-10 / 35%で +28" },
    { id: "dividend-panda", type: "dividend_like", unlockDay: 5, cost: 8, dailyDividend: 3, bankruptcyRate: 0.2, shortLabel: "パンダ組合の話", detailLabel: "-8 / 毎日 +3 / 倒産しやすい" },
    { id: "dividend-koguma", type: "dividend_like", unlockDay: 5, cost: 7, dailyDividend: 2, bankruptcyRate: 0.08, shortLabel: "コグマ商会の話", detailLabel: "-7 / 毎日 +2 / 倒産しにくい" }
  ]
};

const MILESTONE_UNLOCKS = {
  5: ["新しいあずけ先が見つかりました", "新しい前借りの約束が増えました", "配当の話を聞けるようになりました", "うまそうな話を聞けるようになりました"],
  10: ["長いあずけ先が見つかりました", "重い前借りの約束が増えました", "大きな話を聞けるようになりました", "夜の市場のうわさを聞きました"],
...
const newState=()=>({version:2,status:"idle",day:0,broughtIn:0,hand:0,totalSnackPaid:0,actionsLeft:ACTIONS_PER_DAY,animals:1,lends:[],borrows:[],investReturns:[],dividendInvestments:[],investResults:[],actionLogs:[],tired:false,lastDayIncoming:0,lastDayRepay:0,unlockedMilestones:[]});
const appendLog=(s,m)=>{s.actionLogs.unshift(`Day${s.day}: ${m}`);s.actionLogs=s.actionLogs.slice(0,40);};
function showForestToast(message){if(!dom.toast)return;dom.toast.textContent=message;dom.toast.hidden=false;dom.toast.classList.remove("is-visible");void dom.toast.offsetWidth;dom.toast.classList.add("is-visible");if(forestToastTimer)clearTimeout(forestToastTimer);forestToastTimer=setTimeout(()=>{dom.toast.classList.remove("is-visible");dom.toast.hidden=true;},TOAST_DURATION_MS+200);} 
function checkMilestoneUnlocks(s){if(!Array.isArray(s.unlockedMilestones))s.unlockedMilestones=[];const key=String(safeInt(s.day,0));if(!MILESTONE_UNLOCKS[key]||s.unlockedMilestones.includes(key))return;MILESTONE_UNLOCKS[key].forEach((msg)=>appendLog(s,msg));s.unlockedMilestones.push(key);showForestToast("新しい選択肢が増えました");}
function showDayToast(s){if(s.day>1)showForestToast(`${s.day}日目`);} 
function loadForestLifeState(){if(!safeStorage)return newState();let p=null;try{p=JSON.parse(safeStorage.getItem(STORAGE_KEYS.state)||"null");}catch{}const b=newState();if(!p||typeof p!=="object")return b;Object.assign(b,{version:2,status:["playing","ended"].includes(p.status)?p.status:"idle",day:safeInt(p.day,0),broughtIn:safeInt(p.broughtIn,0),hand:safeInt(p.hand,0),totalSnackPaid:safeInt(p.totalSnackPaid,0),actionsLeft:Math.min(ACTIONS_PER_DAY,safeInt(p.actionsLeft,ACTIONS_PER_DAY)),tired:Boolean(p.tired),lastDayIncoming:safeInt(p.lastDayIncoming,0),lastDayRepay:safeInt(p.lastDayRepay,0)});
 ...
 b.unlockedMilestones=Array.isArray(p.unlockedMilestones)?p.unlockedMilestones.map((x)=>String(x)):[];
...
function doLend(s,id){const p=getUnlockedPlans("lend",s).find(x=>x.id===id);if(!p){appendLog(s,"古い貸付プランは使えませんでした。");return;}if(s.actionsLeft<=0||s.hand<p.cost)return; s.hand-=p.cost;s.lends.push({day:s.day+p.dueDays,amount:p.returnAmount});appendLog(s,`${p.cost}枚を森の仲間にあずけました。${p.dueDays}日後に${p.returnAmount}枚で返ってきます。`);consumeAction(s);}
function doBorrow(s,id){const p=getUnlockedPlans("borrow",s).find(x=>x.id===id);if(!p){appendLog(s,"古い前借りプランは使えませんでした。");return;}if(!p||s.actionsLeft<=0)return; s.hand+=p.gain;s.borrows.push({day:s.day+p.dueDays,amount:p.repayAmount});appendLog(s,`森の倉庫から${p.gain}枚を前借りしました。${p.dueDays}日後に${p.repayAmount}枚返します。`);consumeAction(s);}
function doDividendInvestment(s,id){const p=getUnlockedPlans("invest",s).find(x=>x.id===id&&x.type==="dividend_like");if(!p){appendLog(s,"古い配当プランは使えませんでした。");return;}if(s.actionsLeft<=0||s.hand<p.cost)return; s.hand-=p.cost;s.dividendInvestments.push({id:p.id,label:p.shortLabel.replace("の話",""),dailyDividend:p.dailyDividend,bankruptcyRate:p.bankruptcyRate,riskLabel:p.id==="dividend-panda"?"倒産しやすい":"倒産しにくい"});appendLog(s,`${p.shortLabel}に${p.cost}枚使いました。続くかぎり、毎日${p.dailyDividend}枚の配当があります。`);consumeAction(s);}
function doInvest(s,id){const p=getUnlockedPlans("invest",s).find(x=>x.id===id);if(!p){appendLog(s,"古い投資プランは使えませんでした。");return;}if(s.actionsLeft<=0||s.hand<p.cost)return; if(p.type==="dividend_like"){doDividendInvestment(s,id);return;} s.hand-=p.cost;const success=Math.random()<p.successRate;const gain=success?p.successReturn:p.failReturn;s.hand+=gain;let msg="";let toast="";if(success){msg=p.id==="invest-small"?"あやしいクマさんの小さな話は当たりでした。3枚が7枚になりました。":p.id==="invest-mid"?"うまそうな話は当たりでした。6枚が14枚になりました。":"大きな話は当たりでした。10枚が28枚になりました。";toast=`当たり。${p.cost}枚が${gain}枚になりました。`;}else{msg=p.id==="invest-small"?"あやしいクマさんの小さな話は外れました。3枚は戻ってきませんでした。":p.id==="invest-mid"?"うまそうな話は外れました。6枚は戻ってきませんでした。":"大きな話は外れました。10枚は戻ってきませんでした。";toast=`外れ。${p.cost}枚は戻ってきませんでした。`;} s.investResults.unshift(`Day${s.day}: ${msg}`);s.investResults=s.investResults.slice(0,30);appendLog(s,msg);showForestToast(toast);consumeAction(s);}
